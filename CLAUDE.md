...
**목표:** 타로 카드 이미지 22장+뒷면을 major_00_fool 표준 양식(1024x1536)으로 통일하고 QA/반영을 자동화한다.

**트리거:** "타로 QA", "타로 카드 검사", "타로 프롬프트", "카드 인입 검사", "타로 반영", "카드 양식 점검" 등 타로 이미지 자산 작업 요청 시 `tarot-card-pipeline` 스킬을 사용하라. 타로 화면/로직 개발은 대상 아님.

**변경 이력:**
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-07-03 | 초기 구성 | 에이전트 3 + 스킬 1 + QA 스크립트 | 카드 양식·크기 불일치 13장 표준화 |
| 2026-07-03 | 모델 차등 배분 (qa=sonnet, prompt=opus, integrator=haiku) | 에이전트 3 + SKILL.md | 작업 성격별 비용/품질 최적화 |
| 2026-07-03 | 리사이즈 우선 시도 규칙 추가 (Phase C 1.5) | SKILL.md | 크기만 부적합한 9장 중 8장이 재생성 없이 리사이즈로 해결됨 |
